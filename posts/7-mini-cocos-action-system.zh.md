...
mini-cocos 的 Action 系统几乎是直接搬 cocos2d-x 的设计，但写过一遍才发现这套设计**真正聪明的地方在哪一个 API 形状上** —— 不是 `Sequence`，不是 `Spawn`，而是 `update(float t)` 这一个函数的契约："t 永远是 [0, 1] 的归一化时间"。这一条约束是整个系统能 N 层嵌套组合的根本原因。
