# 在没有 libc、没有 std 的地方写现代 C++：Zonix 的 freestanding 内核
...
"内核要用裸 C 写"是一种很顽固的迷信。Zonix 整个内核是 C++17 freestanding——没有 libc、没有 STL、没有异常、没有 RTTI，但**模板、RAII、`constexpr`、`[[nodiscard]]`、命名空间一个没少**。这一篇讲在这种"半条腿"的 C++ 环境里，怎么把缺的那半条腿（运行时支撑）自己补出来，以及为什么这些零开销抽象在内核里比在应用层更值。
...
> 经验：**换一套编译器，是一种几乎免费的 fuzzing。** 不同编译器对"未定义/未指定行为"会做出完全不同但都合法的选择。你那些"在 GCC 下恰好能跑"的代码，本质上是依赖了一组没写进标准的隐性假设。换 Clang，等于让另一组合法假设重新审视一遍全部代码——它不保证发现所有问题，但它发现的每一个，都是真实存在、迟早会爆的隐患。我现在把"定期换编译器编一遍"当成一种常规体检。
...
- 2026-05-22：[`56af896`](https://github.com/leafvmaple/zonix-plus/commit/56af896) 给 `ENSURE` 加变参重载（单参默认 `Error::Invalid`，见 §5）；[`295581b`](https://github.com/leafvmaple/zonix-plus/commit/295581b) 用它批量简化 `exec`/FAT 的错误处理。另外 [`dd6ccee`](https://github.com/leafvmaple/zonix-plus/commit/dd6ccee) 把 ELF 校验封装进 `ElfHdr::is_valid()`/`is_executable()` 成员函数、[`5f15c72`](https://github.com/leafvmaple/zonix-plus/commit/5f15c72) 给一批布尔访问器加 `[[nodiscard]]`——都是"freestanding 里照样用现代 C++"的延续（关联 [#18](https://github.com/leafvmaple/blog/issues/18)）。
...
- 2026-03-12：[`9fae90c`](https://github.com/leafvmaple/zonix-plus/commit/9fae90c) 工具链整体迁移到 Clang/LLD/LLVM 并暴露 `switch_to` bug（见 §6 / [#12](https://github.com/leafvmaple/blog/issues/12)）；新增 `cxxrt.cpp` 收纳运行时桩 + non-inline 的 `new`/`delete`（见 §2/§3）。[`1437166`](https://github.com/leafvmaple/zonix-plus/commit/1437166) UEFI 改用 `clang --target=x86_64-pc-windows-msvc`。
- 2026-03-05：[`2e809ca`](https://github.com/leafvmaple/zonix-plus/commit/2e809ca) 用 `inline constexpr`/inline 函数替换宏，并补架构最优 memops（见 §6）。
- 2026-02-12：[`7138771`](https://github.com/leafvmaple/zonix-plus/commit/7138771) 把各子系统封进命名空间、把 kernel 基础库归位到 `lib/`；[`b69882e`](https://github.com/leafvmaple/zonix-plus/commit/b69882e) v0.9.0 一轮清零所有编译/链接警告（见 §6）。
