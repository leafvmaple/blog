# 信頼できないコードを ring 3 へ招き入れる：Zonix のユーザーモード ELF 実行とシステムコール ABI
...
これまで Zonix で走るのはすべて**カーネルスレッド**でした —— カーネルと同特権レベル（ring 0 / EL1）・同アドレス空間で、本質的に「私が信頼するコード」です。この記事は質的な転換点：**信頼できない、ディスク上のある ELF ファイルから来たコードを、隔離されたアドレス空間に置き、ring 3 へ降格して走らせ、システムコールという狭い窓口からしかカーネルへ戻れないようにする。**

この転換は密に噛み合う四つの部分から成ります：ユーザーアドレス空間の構築、ELF のロード、特権境界の越境、そして「ユーザーがカーネルへ渡すあらゆるポインタは悪意あり得る」への防御。それはまた [#12](https://github.com/leafvmaple/blog/issues/12) で埋めた継ぎ目の実現でもあります —— 当時「将来のユーザープロセスは同じ `trapret` 経路を再利用する」と書いた、それが今やって来ました。
...
> この節はこの記事で私が最も強調したいこと：**ユーザーモードを導入したその瞬間、カーネルにまったく新しい、最も危険な入力の類が増える —— 信頼できないプロセスから、システムコール境界を通って渡されるデータ。** それ以前 Zonix の全入力は私自身が書いたカーネルコードから来た；`exec` で最初のユーザープログラムを動かす瞬間から、「ユーザーポインタを決して信じない」が常に張り詰めていなければならない規律になった。教育的カーネルがこの線を明確に引くことは、システムコールをいくつか多く実装するよりずっと重要です。
...
- 2026-05-22：[`551394f`](https://github.com/leafvmaple/zonix-plus/commit/551394f) exec エンドツーエンドテストが単一ディスクの目標（aarch64/riscv64 の SDHCI）で優雅にスキップし、x86 BIOS/UEFI カバレッジを保つ；[`0b7e929`](https://github.com/leafvmaple/zonix-plus/commit/0b7e929) `GptHeader` を完全な 512 バイトセクタに揃え、`find_partition_start` が直接構造体へ読み込む（[#14](https://github.com/leafvmaple/blog/issues/14) の GPT/パーティションテーブルに関連）。
- 2026-05-22：[`9a321a9`](https://github.com/leafvmaple/zonix-plus/commit/9a321a9) zcc コンパイラサブモジュール + userdata イメージをビルドパイプラインへ統合（§6 参照）；[`dd98ccd`](https://github.com/leafvmaple/zonix-plus/commit/dd98ccd) exec 統合テストを追加し、fork/load/run 全経路を検証。
- 2026-05-22：本サブシステム初稿。[`56af896`](https://github.com/leafvmaple/zonix-plus/commit/56af896) syscall ABI ヘッダを抽出（§4 参照）；[`67608c2`](https://github.com/leafvmaple/zonix-plus/commit/67608c2) `EM_CURRENT` アーキ別 ELF マシン型を追加（§2 参照）；[`dd6ccee`](https://github.com/leafvmaple/zonix-plus/commit/dd6ccee) ELF 検証を `ElfHdr` メンバ関数へ封装；[`295581b`](https://github.com/leafvmaple/zonix-plus/commit/295581b) exec/FAT を `ENSURE`/`TRY` でエラー処理を簡素化（[#17](https://github.com/leafvmaple/blog/issues/17) 参照）。
