...
なぜ独立した一つ、しかも「純粋 C マクロ、アセンブリから include 可能」と限定するか？システムコール番号は**三者を跨ぐ契約**だからです：カーネルのディスパッチャ、ユーザープログラムの libc 風ラッパー、zcc ランタイムのアセンブリスタブ、三者は「4 番は write」に完全に一致せねばならない。どれか一方が番号を間違えれば、wrong syscall が静黙に誤った分岐へ走る。それを純粋 C マクロの単一ヘッダにし、`.S` から直接 include できるようにすることで、**三者が各自定数を持ち徐々にずれる可能性を根本から消す**。これは [#14](https://github.com/leafvmaple/blog/issues/14) の `BootInfo` が bootloader とカーネルの共有契約であるのと同じ発想 —— **インターフェース契約は唯一の物理的出所を持て**。

> **「物理的に同じファイルを include」についての誠実な補足**：zcc は独立リポジトリで Zonix のパスへ逆依存できないため、実際には `zcc/src/runtime/syscall.h` と `zonix-plus/include/abi/syscall.h` は**二つの物理コピー**（番号は同形、header guard / 空白は異なる）であり、[`scripts/check_syscall_abi_sync.sh`](https://github.com/leafvmaple/zonix-plus/blob/main/scripts/check_syscall_abi_sync.sh) が `make user` 時に両者の `#define NR_*` / `*_FD` 行を自動 diff して同期を守る。「二つの物理ファイル、一つの論理契約、自動化兜底」 —— これは zcc 側の視点、完全な解きほぐしは [#21](https://github.com/leafvmaple/blog/issues/21) で。
...
最後のピース：`exec` がロードする ELF は誰がコンパイルしたのか？ Zonix が使うのは**自作の C コンパイラ zcc**（[`9a321a9`](https://github.com/leafvmaple/zonix-plus/commit/9a321a9) でサブモジュールとしてビルドパイプラインへ統合）。`user/Makefile` は `user/zcc/test/*.c` を zcc でコンパイルし、FAT32 8.3 短ファイル名互換の ELF を生成（`Z` プレフィックスで手書きアセンブリ版ユーザープログラムとの命名衝突を避ける）、`user/hello/hello.S` のようなアセンブリ版ユーザープログラムと一緒に userdata ディスクイメージへ詰める；カーネル起動後このディスクを `/mnt` へマウントし、`exec("/mnt/ZHELLO.ELF")` でそれを走らせる。

zcc は既に完全なサブシリーズに独立 —— **メインインデックス [#20](https://github.com/leafvmaple/blog/issues/20)**（プロジェクト骨格 + 3 つの工学的決定 + 6 つの落地後の事実）、記事 [#21 ABI 継ぎ目](https://github.com/leafvmaple/blog/issues/21)（§4 の「二つの物理ファイル、一つの論理契約、自動化兜底」の全細部を解きほぐす、crt0 / linker.ld / 端から端まで流れ図を含む）、[#22 LLVM codegen](https://github.com/leafvmaple/blog/issues/22)（Koopa バックエンド削除で正味 -1,414 行の減算リファクタ + char-i8 + opaque pointer 時代の GEP 二つの形）、[#23 C0 進化](https://github.com/leafvmaple/blog/issues/23)（SysY から `printf("Hello\n")` を編める最小フロントエンドへ + LALR dangling-else + 配列 decay フラグ）。

ここで補足するに値するのは exec 経路がエラークリーンアップ保証に RAII を多用すること —— `KernelBuf`（自動 `kfree`）、`OpenFile`（自動 `vfs::close`）、どの段階で失敗してもデストラクタが申請済みリソースをきれいに回収する。これこそ [#17](https://github.com/leafvmaple/blog/issues/17) で述べた「freestanding カーネルでも RAII でリソースを締める」の新経路での応用です。
...
- 2026-05-23：§6 を「もう一つの大きな話題で単独記事に値する」から完全な zcc シリーズ案内（メインインデックス [#20](https://github.com/leafvmaple/blog/issues/20) + 記事 [#21](https://github.com/leafvmaple/blog/issues/21)/[#22](https://github.com/leafvmaple/blog/issues/22)/[#23](https://github.com/leafvmaple/blog/issues/23)）に拡張。§4 ABI 段に「二つの物理ファイル、一つの論理契約」の誠実な振り返りを追加（新規追加された [`scripts/check_syscall_abi_sync.sh`](https://github.com/leafvmaple/zonix-plus/blob/main/scripts/check_syscall_abi_sync.sh) を指す）、「物理的に同じファイルを include」という元の不正確な記述を訂正。
