# CJK Label がロード画面を固める真因は文字数ではなく、文字ごとに 1 枚のテクスチャ
...
mini-cocos のリソースパイプラインは「正しく作れば透明、誤れば至る所で死ぬ」2 箇所で姿を現す：

```
$ wc -l src/base/ZCFontAtlas.cpp src/platform/ZCFileUtils.cpp src/platform/win32/ZCFileUtilsWin32.cpp
   378 src/base/ZCFontAtlas.cpp                  # FontAtlas + stb_truetype 接続
   290 src/platform/ZCFileUtils.cpp              # クロスプラットフォーム FileUtils
   297 src/platform/win32/ZCFileUtilsWin32.cpp   # win32 実装
   965 total
```

2 件とは：**フォント描画をどう殺さないか**（FontAtlas の文字オンデマンド / インクリメンタルラスタライズ、`stb_truetype` の `stbtt_GetCodepointBitmap` で単文字ラスタライズ + 共有 atlas）と、**多言語 + 多解像度 + プレイヤー mod 下で「ファイルを 1 つ読む」をどう管理するか**（FileUtils の searchPath 列、複数 root への順序フォールバック）。
...
- 2026-05-22：[`fc43261`](https://github.com/leafvmaple/mini-cocos/commit/fc43261) Label + Lua/UI binding のマイナー清掃（Label ヘッダの使われない friend 2 つ削除、UIButton を Label の字サイズ API 名に乗り換え）。
- 2026-05-22：[`155f650`](https://github.com/leafvmaple/mini-cocos/commit/155f650) `FontAtlasCache` を新設 —— `FontAtlas` の上に (face + 字サイズ) を key にした再利用キャッシュ。以前は同一 ttf でも字サイズごとに atlas が分裂され、半透明 UI の多さまずまなサイズと重なると atlas page が 8+ に腣れることがあった。cache 導入後 2~3 枚に收束。この cache は Renderer の Label 跨バッチマージを成らしめる前提でもある（同フォント同サイズの 2 Label が atlas page を共有して初めて cmd マージが成り立つ）。伴って Label は「ページ単位で emit」に書き換え、詳細は [#6 レンダリングイテレーション記録](https://github.com/leafvmaple/blog/issues/6) 参照。
- 2026-05-22：[`6e06290`](https://github.com/leafvmaple/mini-cocos/commit/6e06290) + [`1e8f941`](https://github.com/leafvmaple/mini-cocos/commit/1e8f941) Label と FontAtlas の構造を cocos2d-x 本家よりに再編成 —— 元は Label 内部で書いていた文字レイアウトを FontAtlas 側に差し戻した（「どの字を持ち、その advance/uv は何か」を 1 箇所に集約）、Label は「atlas へのクエリ結果から頂点生成」だけを担う。この一刀により「フォント交換」「shader 交換」「リッチテキスト」のコストが一桁下がった。

---

*リポジトリ：[leafvmaple/mini-cocos](https://github.com/leafvmaple/mini-cocos)。本記事は [mini-cocos シリーズ](https://github.com/leafvmaple/blog/issues/2) の一篇；「統一入口」哲学は EventDispatcher の `dispatch()` も同様（[#5](https://github.com/leafvmaple/blog/issues/5)）。*
